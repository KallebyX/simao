...
const AppState = {
  ...
  render() {
    ...
    if (rootElement) {
      const viewFunction = Views[this.currentView] || Views.login;
      const viewContent = viewFunction.call(Views); // Fix context binding
...
const API = {
  ...
  async register(userData) {
    try {
      ...
      // Fallback: simulate registration for demo
      console.warn('Backend offline, using demo mode');
      return {
  ...
  async getTickets() {
    try {
      ...
      // Demo data
      return [
        { id: 1, subject: 'Problema com WhatsApp', contact: 'João Silva', status: 'open', updatedAt: new Date() },
        { id: 2, subject: 'Configurar bot', contact: 'Maria Santos', status: 'pending', updatedAt: new Date() },
        { id: 3, subject: 'Dúvida sobre preços', contact: 'Pedro Costa', status: 'closed', updatedAt: new Date() }
      ];
    }
  },
  ...
  async getContacts() {
    try {
      ...
      // Demo data
      return [
        { id: 1, name: 'João Silva', phone: '11999999999', email: 'joao@email.com', lastMessage: 'Olá!' },
        { id: 2, name: 'Maria Santos', phone: '11888888888', email: 'maria@email.com', lastMessage: 'Preciso de ajuda' },
        { id: 3, name: 'Pedro Costa', phone: '11777777777', email: 'pedro@email.com', lastMessage: 'Obrigado!' }
      ];
    }
  },
  ...
  async getCampaigns() {
    try {
      ...
      // Demo data
      return [
        { id: 1, name: 'Black Friday 2024', status: 'active', sent: 1500, opened: 890 },
        { id: 2, name: 'Promoção Natal', status: 'scheduled', sent: 0, opened: 0 },
        { id: 3, name: 'Pesquisa Satisfação', status: 'completed', sent: 2000, opened: 1200 }
      ];
    }
  },
  ...
  async getWhatsAppSessions() {
    try {
      ...
      // Demo data
      return [
        { id: 1, name: 'Atendimento 1', status: 'CONNECTED', messagesCount: 89 },
        { id: 2, name: 'Vendas', status: 'CONNECTED', messagesCount: 156 },
        { id: 3, name: 'Suporte', status: 'DISCONNECTED', messagesCount: 0 }
      ];
    }
  }
};
      ...
      ${this.layout(`
        <div style="padding: 2rem;">
          <div style="margin-bottom: 2rem;">
            ...
            ${AppState.token?.startsWith('demo-token') ? `
              <div style="background: #fef3c7; border: 1px solid #f59e0b; border-radius: 0.5rem; padding: 1rem; margin-bottom: 1rem;">
                <div style="display: flex; align-items: center;">
                  <span style="margin-right: 0.5rem;">⚠️</span>
                  <div>
                    <p style="font-weight: 500; color: #92400e; margin: 0 0 0.25rem 0;">Modo Demo Ativo</p>
                    <p style="font-size: 0.875rem; color: #78350f; margin: 0;">Backend offline - usando dados de demonstração. Todas as funcionalidades estão disponíveis.</p>
                  </div>
                </div>
              </div>
            ` : ''}
